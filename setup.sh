...
sudo squid -z -f /etc/squid/squid.conf > /dev/null
sudo squid -z -f /etc/squid/squidpt.conf > /dev/null

squid -f /etc/squid/squid.conf
squid -f /etc/squid/squidpt.conf

#run
#service squid restart && update-rc.d squid defaults
#service squidpt restart && update-rc.d squidpt defaults

sudo update-rc.d squidpt defaults
